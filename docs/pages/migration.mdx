...
Also, the server lambda's default build path has changed from `.open-next/server-function` to `.open-next/server-functions/default`, so any supporting infrastructure will need to be reconfigured.
